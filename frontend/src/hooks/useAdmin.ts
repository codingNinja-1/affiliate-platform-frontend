...
export function usePaymentSettings() {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err instanceof Error ? err : new Error('Unable to load payment settings'));
      setSettings(null);
    } finally {
      setIsLoading(false);
    }
    ...
    async (newSettings: Partial<PaymentSettings>) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(errData?.message || 'Failed to save payment settings');
        }
  ...
  return { settings, isLoading, error, isSaving, refetch: fetchSettings, saveSettings };
}
