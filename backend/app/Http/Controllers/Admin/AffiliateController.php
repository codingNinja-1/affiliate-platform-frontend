...
class AffiliateController extends Controller
{
    ...
    public function approve($id)
    {
        ...
        try {
            if ($affiliate->email) {
                ...
                Mail::to($affiliate->email)->queue(new AffiliateApprovedMail($affiliate));
                Log::info('Affiliate approval email queued', ['affiliate_id' => $affiliate->id]);
            }
        } catch (\Exception $e) {
            Log::warning('Failed to queue affiliate approval email', [
    ...
    public function reject($id, Request $request)
    {
        ...
        try {
            if ($affiliate->email) {
                DynamicMailService::configure();
                Mail::to($affiliate->email)->queue(new AffiliateDeclinedMail($affiliate, $reason));
                Log::info('Affiliate rejection email queued', ['affiliate_id' => $affiliate->id]);
            }
        } catch (\Exception $e) {
            Log::warning('Failed to queue affiliate rejection email', [
                'affiliate_id' => $affiliate->id,
                'error' => $e->getMessage(),
            ]);
