...
// Cache rates in module scope so they survive re-renders and are shared across hook instances
...
export function useCurrencyConversion(refreshTrigger = 0, selectedCurrency = 'NGN') {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch('/api/affiliate/converted-amounts', {
